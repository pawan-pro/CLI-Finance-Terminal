...
import { Bot, Send, Loader2, Sparkles, Filter, Activity } from 'lucide-react';
...
export const AIAssistant: React.FC<AIAssistantProps> = ({ marketContext }) => {
  ...
  const generateBriefing = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`${CONFIG.BACKEND_API_BASE}/api/research`);
      if (!response.ok) {
        if (response.status === 429) {
          addBriefingMessage('Strategist is reviewing deep-book liquidity. Retry in 60s.');
          return;
        }
      ...
      if (data.error && data.error.includes('429')) {
        ...
        addBriefingMessage(`📊 STRATEGIC BRIEFING\n\n${data.report}`);
      }
    } catch (error) {
      addBriefingMessage('Strategist is reviewing deep-book liquidity. Retry in 60s.');
    } finally {
  ...
  const addBriefingMessage = (content: string) => {
  ...
  return (
    <div className="flex flex-col h-full bg-nexus-900 border-l border-slate-800">
      ...
      <div className="p-2 bg-nexus-800/50 border-b border-slate-800">
        <button
          onClick={generateBriefing}
          disabled={isLoading}
          className="w-full flex items-center justify-center space-x-2 py-2 bg-nexus-900 border border-yellow-600/50 hover:border-yellow-500 hover:bg-nexus-800 transition-all rounded shadow-lg shadow-yellow-900/10 group disabled:opacity-50"
        >
          <Activity size={14} className="text-yellow-500 group-hover:animate-pulse" />
          <span className="text-[10px] font-mono font-bold text-yellow-500 uppercase tracking-widest">
            {isLoading ? 'ANALYZING...' : 'GENERATE STRATEGIC BRIEFING'}
          </span>
        </button>
      </div>
        ...
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[95%] rounded-lg p-3 text-xs leading-relaxed ${
              msg.role === 'user' 
                ? 'bg-nexus-700 text-slate-100 rounded-tr-none font-sans' 
                : 'bg-nexus-800/80 border border-slate-700 border-l-2 border-l-yellow-600/50 text-slate-300 rounded-tl-none font-normal font-sans'
            }`} style={{ fontFamily: '"Inter", sans-serif' }}>
              {msg.content.split('\n').map((line, i) => {
                // Check for headers (backend uses # and ##)
                const isHeader = line.startsWith('#');
                if (isHeader) {
                  return (
                    <h4 key={i} className="text-[11px] font-mono font-bold text-yellow-500 uppercase tracking-widest mt-5 mb-2 border-b border-yellow-500/10 pb-1">
                      {line.replace(/#/g, '').trim()}
                    </h4>
                  );
                }
                return (
                  <p key={i} className={line.startsWith('•') || line.startsWith('-') ? 'ml-3 mb-1.5' : 'mb-3 last:mb-0'}>
                    {line}
                  </p>
                );
              })}
